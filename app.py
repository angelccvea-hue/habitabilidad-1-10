"""
Punto de entrada del BI Streamlit — Cruce 1×10 × Habitable.

Flujo de ``main()`` (orden fijo; no reordenar sin revisar seguridad):

1. ``inject_executive_css`` — tema visual.
2. ``require_login`` — bloquea todo hasta usuario + TOTP válidos.
3. Comprueba cifrado en reposo (producción / archivos ya cifrados).
4. Panel de carga (solo admin) si faltan parquet.
5. ``load_data`` — lee parquet (vía ``secure_io``).
6. ``apply_sol_pii_policy`` — quita contacto al rol ejecutivo.
7. Sidebar: sesión, política tiles, navegación ``nav_schema``.
8. Despacho a ``page_*`` según el ítem activo.

Ejecutar::

    streamlit run app.py

Documentación para revisores: ``docs/00-GUIA-DE-LECTURA.md``.
"""
