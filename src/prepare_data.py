...
Corazón del dominio de datos (no es seguridad; produce los insumos del tablero).

Pasos de ``run_pipeline``:
1. Lee Excel/CSV (vía ``secure_io`` si están cifrados).
2. Limpia coordenadas, texto (mojibake), territorios.
3. Dedup 1×10 (``dedupe_1x10``) → representantes.
4. Matching espacial + nombre vs Habitable (BallTree haversine).
5. Escribe ``data/processed/solicitudes.parquet``, ``inspecciones.parquet``,
   ``summary.json``.

Parámetros de radio/scores: ``config.toml`` sección ``[matching]``.
Documentación: ``docs/04-DATOS-Y-PIPELINE.md``.
"""
