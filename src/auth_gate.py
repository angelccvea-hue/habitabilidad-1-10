...
Capa de seguridad #1 — nadie ve datos ni UI de negocio sin sesión válida.

Responsabilidades:
- Pantallas de bootstrap (primer admin), login y enrolamiento 2FA.
- Sesión Streamlit: ``bi_authenticated``, ``bi_username``, ``bi_role``.
- Helpers de autorización: ``can_upload``, ``can_see_contact``, ``can_manage_users``.
- Panel admin (alta de usuarios) + expander de auditoría reciente.
- Registro de eventos login/logout/fail vía ``audit_log``.

Almacenamiento de usuarios: ``auth_users`` → ``data/auth/users.json`` (cifrable).
Ver también: ``docs/03-SEGURIDAD.md``.
"""
