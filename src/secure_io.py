...
Capa de seguridad #4 — protege disco del servidor si alguien copia archivos sin clave.
...
API principal: ``read_bytes`` / ``write_bytes``, ``read_parquet`` / ``write_parquet``,
``read_json`` / ``write_json``. Migración de archivos existentes:
``scripts/encrypt_data_at_rest.py``.
"""
