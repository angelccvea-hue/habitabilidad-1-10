...
Capa de seguridad #6 — evitar que OSM/Carto/Esri vean el área operativa.
...
Usado por ``map_robust``, ``pages_abordaje`` y ``pages_nasa``.
"""
    ...
    if public_tiles_allowed():
        ...
        for k, v in public.items():
            if k not in out:
                out[k] = v
    if not out:
        out[NONE_KEY] = (NONE_TILES, "Sin tiles externos", "Sin base")
    return out
