...
Modelo de identidad del propio tablero Streamlit.

- Roles: ``ejecutivo`` (sin PII) · ``operador`` (colas/contacto) · ``admin`` (carga + usuarios).
- Claves: bcrypt (cost 12). Nunca se guardan en claro.
- 2FA: secreto TOTP por usuario; QR al enrolar (issuer configurable en código).
- Persistencia: ``data/auth/users.json`` vía ``secure_io`` (cifrado si hay BI_DATA_KEY).
"""
...
TOTP_ISSUER = "Habitabilidad-1x10"
