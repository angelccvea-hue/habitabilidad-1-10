...
Capa de seguridad #3 — trazabilidad de quién hizo qué.

Registro en ``data/audit/audit.jsonl`` (una línea JSON por evento).
No se versiona en Git. Fallos de disco no tumbaron la app (swallow).

Eventos típicos: ``login_ok``, ``login_fail``, ``logout``, ``user_create``,
``download``, ``pipeline_ok``, ``pipeline_fail``, ``upload_reject``.
"""
